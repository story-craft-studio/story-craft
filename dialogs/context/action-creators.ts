...
export function addPassageEditors(
	...
	return (dispatch, state) => {
		...
		const passageEditStackIndex = currentState.activeDialogs.findIndex(
		...
		if (passageEditStackIndex !== -1) {
			...
			const existing: string[] =
				currentState.activeDialogs[passageEditStackIndex].props?.passageIds ?? [];
			...
			dispatch({
				...
				props: {
					...currentState.activeDialogs[passageEditStackIndex].props,
...
export function removePassageEditors(
	...
	return (dispatch, state) => {
		...
		const updatedPassageIds = currentState.activeDialogs[
