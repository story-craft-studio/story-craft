...
export function passageToTwee(passage: Passage) {
	...
	const escapedText = escapeForTweeText(passage.commands?.[0]?.content?.text);
...
export function passageFromTwee(source: string): Omit<Passage, 'story'> {
	console.log('passageFromTwee ? ', source);
...
export function storyFromTwee(source: string) {
	...
	story.passages = story.passages.filter(passage => {
		...
		if (isScript) {
			story.script += passage.commands?.[0]?.content?.text + '\n';
		} else if (isStylesheet) {
			story.stylesheet += passage.commands?.[0]?.content?.text + '\n';
		}
	...
	if (titlePassageIndex !== -1) {
		story.name = story.passages[titlePassageIndex].commands?.[0]?.content?.text.trim();
	...
	if (dataPassageIndex !== -1) {
		...
		try {
			const {
				...
				zoom
			} = JSON.parse(dataPassage.commands?.[0]?.content?.text);
			...
			console.warn(`Couldn't parse story data`, dataPassage.commands);
		}
	} else {
