...
export function parseLinksFromCommands(commands: PassageCommand[], internalOnly?: boolean): string[] {
	let links: string[] = [];
	commands.forEach(eachCmd => {
		let result = uniq(
			extractLinkTags(eachCmd?.content?.text || '')
				.map(removeEnclosingBrackets)
				.map(removeSetters)
				.map(extractLink)
				.filter(nonEmptyLinks)
