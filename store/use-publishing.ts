...
export function usePublishing(): UsePublishingProps {
	...
	return {
		...
		proofStory: React.useCallback(
			async storyId => {
				if (!storyId) {
					throw new Error('No story id to publishStory ' + storyId);
				}
				...
				return await publishStoryWithFormat(
					story,
		...
		publishStory: React.useCallback(
			async (storyId, publishOptions) => {
				console.log('publishStory', storyId, publishOptions);
				if (!storyId) {
					throw new Error('No story id to publishStory ' + storyId);
				}
				const story = storyWithId(stories, storyId);
				...
				// Disable remix if requested
				const originalRemix = story.remixEnabled;	// Save original remix state
				if (publishOptions?.forceDisableRemix) {
					story.remixEnabled = false;				// Disable remix
				}

				const result = await publishStoryWithFormat(
					story,
					formatProperties.source,
					getAppInfo(),
					publishOptions
				...
				story.remixEnabled = originalRemix;			// Restore original state
		...
		publishStoryData: React.useCallback(
			(storyId: string) => {
				const story = storyWithId(stories, storyId);
				console.log('usePublishing.publishStoryData');
