...
export function deleteOrphanedPassages(
	...
	return dispatch => {
		const oldLinks = parseLinksFromCommands(oldCommands);
		const newLinks = parseLinksFromCommands(newCommands);
		...
		const passageIds = orphans.reduce<string[]>((result, orphan) => {
			...
			if (
				story.passages.some(
					p => p.id !== passage.id && parseLinksFromCommands(p.commands).includes(orphan)
				)
			) {
				return result;
			}
