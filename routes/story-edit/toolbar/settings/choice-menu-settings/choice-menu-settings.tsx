...
import {ChoiceMenuSettingGroup} from "./choice-menu-setting-group";
...
export interface ChoiceMenuSettingsProps extends DialogComponentProps {
	storyId: string;
}

export function ChoiceMenuSettings(props: ChoiceMenuSettingsProps) {
	const {storyId, ...other} = props;
	const {t} = useTranslation();

	const {showPresetChoiceMenu} = defaultSettings;
	...
	const settings = ChoiceMenuSettingMgr.fromRawObj(story?.storySetting?.choiceMenuSetting, story);
		...
		() => {
			...
			return debounce((newSettings: TwineGameSetting) => {
				...
				try {
					...
					storySetting.choiceMenuSetting = newSettings.toRaw();
					...
					console.error("Failed to save choice menu settings:", error);
					setHasError(true);
				} finally {
					setIsLoading(false);
				}
	...
	return (
		<DialogCard
			...
			headerLabel={t('dialogs.choiceMenuSettings.title', 'Choice Menu Settings')}
		>
			...
			<Box sx={{
				...
				<Box sx={{
					...
					<ChoiceMenuSettingContext.Provider value={{
						...
						{!hasError && groupNames.length === 0 && (
							<Typography color="text.secondary" sx={{ textAlign: 'center', my: 2 }}>
								{t('dialogs.choiceMenuSettings.noSettings', 'No choice menu settings found')}
							</Typography>
						)}
						
						{groupNames.map((groupName, index) => (
							<TwineSettingGroupCard key={groupName} sx={{ mb: index < groupNames.length - 1 ? 3 : 0 }}>
								<ChoiceMenuSettingGroup groupName={groupName}/>
							</TwineSettingGroupCard>
						))}
					</ChoiceMenuSettingContext.Provider>
				</Box>
			...
			{/* Upload preset modal */}
			{showUploadModal && showPresetChoiceMenu && (
				<PresetUploadModal 
