...
import {StartMenuSettingGroup} from "./start-menu-setting-group";
...
export interface StartMenuSettingssProps extends DialogComponentProps {
	storyId: string;
}

export function StartMenuSettings(props: StartMenuSettingssProps) {
	...
	const settings = StartMenuSettingMgr.fromRawObj(story?.storySetting?.startMenuSetting, story);
		...
		() => {
			...
			return debounce((newSettings: TwineGameSetting) => {
				...
				try {
					...
					storySetting.startMenuSetting = newSettings.toRaw();
					...
					console.error("Failed to save start menu settings:", error);
					setHasError(true);
				} finally {
					setIsLoading(false);
				}
		...
		>
			...
			<Box sx={{
				...
				<Box sx={{
					...
					<StartMenuSettingContext.Provider value={{
						...
						{!hasError && groupNames.length === 0 && (
							<Typography color="text.secondary" sx={{ textAlign: 'center', my: 2 }}>
								{t('dialogs.storySettings.noSettings', 'No start menu settings found')}
							</Typography>
						)}
						
						{groupNames.map((groupName, index) => (
							<TwineSettingGroupCard key={groupName} sx={{ mb: index < groupNames.length - 1 ? 3 : 0 }}>
								<StartMenuSettingGroup groupName={groupName}/>
							</TwineSettingGroupCard>
						))}
					</StartMenuSettingContext.Provider>
				</Box>
