...
export function fakeCommands(): PassageCommand[] {
	let cmds: PassageCommand[] = [];
	let count = _.random(0, 20);
	let commandTypes = Object.values(CommandType);

	while (count > 0) {
		let randCmdType = _.sample(commandTypes) as CommandType;
		cmds.push({
			content: {
...
export function fakePrefs(overrides?: Partial<PrefsState>): PrefsState {
	...
	return {
		...
		subDialogWidth: faker.number.int(450),
